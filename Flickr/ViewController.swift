    ...
    private func getFeeds() -> Observable<[Feed]> {
        return flickrHTTPServiceProvider.data(fromUrlString: "https://api.flickr.com/services/feeds/photos_public.gne?tags=food&tagmode=any&format=json&nojsoncallback=1")
            ...
            .map { data in
                ...
                let jsonStr = try JSONSerialization.jsonObject(with: jsonData, options: []) as? [String: Any]
...
extension ViewController {
    ...
    private func setupCellConfiguration () {
        feeds.asObservable()
            ...
            .bind(to: tableView.rx.items(cellIdentifier: "FlickrTableViewCell", cellType: FlickrTableViewCell.self)) {
                [weak self] (index, feed, cell) in
